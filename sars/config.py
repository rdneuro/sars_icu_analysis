...
class GNNMultimodalConfig:
    ...
    sc_metrics: List[str] = field(
        default_factory=lambda: ["sift2", "mean_fa"]
    )

    # FC method
    fc_method: str = "correlation"  # matches file naming: connectivity_correlation.npy
    ...
    def sc_dir(self) -> Path:
        ...
        Returns the DWI pipeline output directory containing per-subject
        ``matrices/`` subdirectories.  Checks both ``data/output/`` and
        ``data/outputs/`` naming conventions.
        """
        for subdir in ["output", "outputs"]:
            p = self.project_root / "data" / subdir / "diffusion" / "subjects"
            if p.exists():
                return p
        # Fallback without diffusion/subjects (some setups)
        for subdir in ["output", "outputs"]:
            p = self.project_root / "data" / subdir
            if p.exists():
                return p
        return self.project_root / "data" / "output" / "diffusion" / "subjects"
