...
node_labels = pd.read_csv('/mnt/nvme1n1p1/sars_cov_2_project/info/atlases/labels_brainnetome.csv')['label_roi'].tolist()
ls_df, ls_10, ls_20 = [], [], []
...
for sub in get_lsubs():
    sc_matrix = get_mtx(sub, 'dmri', 'connectivity_sift2', atlas='brainnetome')
    fc_matrix = get_mtx(sub, 'fmri', 'connectivity_correlation', atlas='brainnetome')  
    ...
    ls_df.append(df)
    fig = plot_sc_fc_decoupling_summary(results, node_labels=node_labels, save_path=f"/mnt/nvme1n1p1/sars_cov_2_project/figs/decoupling_graph/{sub}_decoupling.png")
